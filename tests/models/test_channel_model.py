...
def test_to_json_alias(sample_channel):
    """Ensure to_json and json property behave identically."""
    assert sample_channel.to_json() == sample_channel.to_dict()
    assert sample_channel.json == sample_channel.to_dict()
...
def test_from_json_alias():
    data = {"id": "voice", "type": "oneshot", "size": 3}
    ch1 = Channel.from_dict(data)
    ch2 = Channel.from_json(data)
