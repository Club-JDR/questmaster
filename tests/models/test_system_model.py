...
def test_to_json_and_json_property(sample_system):
    assert sample_system.to_json() == sample_system.to_dict()
    assert sample_system.json == sample_system.to_dict()
...
def test_from_json_alias():
    data = {"id": 3, "name": "Pathfinder", "icon": "/img/pf.png"}
    s1 = System.from_dict(data)
