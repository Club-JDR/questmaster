...
def _build_fake_games() -> list[dict]:
    """Build the demo pages' fake game listings, dated relative to today.

    Computed per call rather than at import time, so the demo sessions stay
    in the future no matter how long the process has been running.

    Returns:
        List of fake game dicts, shaped like ``Game.to_dict()``.
    """
    today = datetime.now()
...
def demo_general():
    ...
    return render_template("games.j2", title="Annonces", games=_build_fake_games())


@demo_bp.route("/demo/inscription/", methods=["GET"])
def demo_register():
    """Render the demo registration page."""
    return render_template("game_details.j2", game=_build_fake_games()[0], is_player=False)
